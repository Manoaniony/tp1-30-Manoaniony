...
public class CustomerManager {
    ...
    /**
     * va exécuter une requête JPQL(langage de requête de JPA) dont le nom est "Customer.findAll", qui est définie dans l'entité Customer déjà écrite (la requête correspond à un "select *" sur les clients)
     * @return 
     */
    ...
    /**
     * donne à gérer à l'entity manager le customer passé en paramètre (méthode merge dont les finesses seront étudiées dans le cours JPA). Au moment du commit qui aura lieu à la fin de la méthode, comme vous le verrez dans le cours sur CDI, les modifications apportées à customer depuis sa création seront enregistrées dans la base de données. Le plus souvent update génèrera un UPDATE SQL dans la base de données.
     * @param customer
     * @return 
     */
    @Transactional
